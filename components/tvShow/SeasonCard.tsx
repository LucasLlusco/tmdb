import { formatDate, formatUserScore, formatYear, isDatePassed } from '@/lib/utils'
    ...
    <div className='flex flex-row gap-[10px] card-boxshadow rounded-[5px]'>
      ...
      <div className={"flex flex-col py-[5px]"}>
        ...
        <div className="flex gap-2 items-center">
          {isDatePassed(season.air_date) && (
            <span className='bg-slate-800 text-white text-xs px-[5px] py-[5px] rounded-[5px] font-bold flex items-center gap-[5px]'>
              ...
              <span>{formatUserScore(season.vote_average)}%</span>
            </span>            
          )}
        ...
        <p className='mt-[10px] text-[14px]'>{isDatePassed(season.air_date) ? (
          `Season ${season.season_number} of ${tvShowName} premiered on ${formatDate(season?.air_date)}`
        ) : (
          `Season ${season.season_number} of ${tvShowName} is set to premiere on ${formatDate(season?.air_date)}`
        )}
        </p>
