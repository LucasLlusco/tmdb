...
const ListCard = ({list, isOwner}: ListCardProps) => {
  ...
  const mediaType = list.mediaTypes![0];
  const mediaId = list.mediaIds![0];
  const { data } = useQuery<Movie | TvShow>({
    ...
    enabled: !!list.mediaIds.length, //only run when there is items.
  });
  ...
  return (
    <div className='flex flex-row card-boxshadow rounded-[5px]'>
      ...
      <div className={"flex flex-col py-[5px] px-[10px] justify-between"}>
        <div className="flex flex-col">
          <p className='text-base font-bold mb-[5px]'>
            <Link href={listPathname} className='link-black'>{list.title}</Link>
          </p>
          <p className='text-xs text-gray-500'>{list.mediaIds?.length} items</p>
